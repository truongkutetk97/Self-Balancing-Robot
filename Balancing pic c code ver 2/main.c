...
void move(double speed, INT minabsspeed,int m);
   ...
   WHILE (1)
   {
      ...
      ELSE
      {
         ...
         IF (RX_Command_Ready == 1) //receive data processing take 4, 44ms
         {
            ...
            // else IF (RxBuffer[1] == '1') {}//move (output, 10, 1) ;
...
void move(double speed, INT minabsspeed,int m)
{
   ...
   IF ( (inputt >= 230)|| (inputt <= 130)|| ( (abs (inputt - setpoint)) <= 0.5)||(m==0))
   {
      speed = 0; dir = 1;
   }
